...
class AutopilotView extends WatchUi.View {
  ...
  function drawChangeHeading(dc) {
    dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_WHITE);
    dc.drawText(width / 2, 45, Graphics.FONT_SYSTEM_TINY, "Change\nHeading",
                (Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER));

    dc.drawText(width / 2, height / 2, Graphics.FONT_NUMBER_THAI_HOT,
                changeHeading,
                (Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER));
  }
...
//    }
    ...
    dc.drawText(width / 2, 165, Graphics.FONT_SYSTEM_MEDIUM, stateName,
...
//    drawRudderAngle(dc, Utils.radiansToDegrees(vessel.rudderAngle));
    ...
    for (var i = 1; i < 8; i += 1) {
      dc.drawLine(0 + xOffsetTens * i, height / 2 - rudderHeight / 2,
                  +xOffsetTens * i, height / 2 + rudderHeight / 2);
    }

    dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_WHITE);
    dc.setPenWidth(4);

    dc.drawLine(0, height / 2 - rudderHeight / 2, width,
                height / 2 - rudderHeight / 2);
    dc.drawLine(0, height / 2 + rudderHeight / 2, width,
                height / 2 + rudderHeight / 2);

    dc.drawLine(width / 2, height / 2 - rudderHeight / 2, width / 2,
                height / 2 + rudderHeight / 2);
  ...
  function drawDataText(dc, x, y, labelText, valueText) {
    dc.drawText(x, y - 2, Graphics.FONT_SYSTEM_XTINY, labelText,
                Graphics.TEXT_JUSTIFY_CENTER);

    dc.drawText(x, y + 28, Graphics.FONT_NUMBER_HOT, valueText,
                Graphics.TEXT_JUSTIFY_CENTER);
  }
  ...
  function drawRudderAngle(dc, rudderAngle) {
    ...
    dc.fillRectangle(width / 2 - xOffset, height / 2 - rudderHeight / 2,
                     width / 2 * percentage, rudderHeight);
  }
}
...
class AutopilotDelegate extends WatchUi.BehaviorDelegate {
  ...
  function onSelect() {
    ...
    var standbyItem =
        new WatchUi.MenuItem("Standby", null, AP_STATE_STANDBY, null);
    ...
    WatchUi.pushView(menu, new AutopilotMenuDelegate(vessel), WatchUi.SLIDE_UP);
